...
- `npm run smoke:skiapi`: passed 13 checks against `http://<side-load-host>:3003/`。
...
- `vite.config.js:8-9` 将 `/api` 和 `/v1` 代理到 `http://<side-load-host>:8080`。
...
- 如果必须临时连远端 side-load，应显式运行 `VITE_PROXY_TARGET=<authorized-backend-url> npm run dev`。
...
4. 如果必须连远端，要求显式 `VITE_PROXY_TARGET=<authorized-backend-url> npm run dev`。
...
- `<side-load-host>` SSH 可达，但该主机不是当前 `skiapi`/NewAPI 生产节点。
...
- `skiapi.dev` DNS 当前解析到 `<production-host-ip>`，不是 `<side-load-host>`。
- `https://skiapi.dev/`、`http://skiapi.dev/api/status`、以 `Host: skiapi.dev` 访问 `<production-host-ip>` 均返回 `502 Bad Gateway`。
- 未执行部署，未持久修改远端；冒烟测试临时 `/tmp/smoke_*` 文件已清理。

下一步需要 `<production-host-ip>` 或实际 `skiapi.dev` 后端主机的 SSH 凭据，才能验证 backend image digest、fork commit、nginx config、安全头和 `/api/assistant/*` 行为。

## 第四轮低风险同步 - 2026-04-28

本轮仅同步本地文档/部署配置到当前 3003 side-load 现实，未连接或修改远端服务器。上面的“生产冒烟测试”结论仍用于说明 `skiapi.dev` 生产域名未验证；当前可用的 SKIAPI 临时入口是 `http://<side-load-host>:3003/`，详见 `SMOKE_TEST_2026-04-28.md` 的 side-load 和 port cutover 记录。

### 已同步

- `scripts/config.example.py`: 默认示例目标从旧 `8080` 改为当前 `<side-load-host>:3003` side-load；补充 `newapi-app-skiapi`、`/opt/skiapi-newapi/data/one-api.db`、`FRONTEND_EXTRA_PORTS=(18080,)`。
...
- `AGENTS.md`、`CLAUDE.md`、`README.md`: 标明 `<side-load-host>:3003` 是当前 SKIAPI side-load，`<side-load-host>:80` 与 `:8080` 仍属于 TH-Platform，不是 SKIAPI 发布目标。
...
- `npm run smoke:skiapi`: pass 13 checks against `http://<side-load-host>:3003/`，最终 SPA shell hash `e97043be211c`。
- `npm run check:ai-security`: pass。
- `npm audit --omit=dev`: pass，0 vulnerabilities。
